...
export default function ProductPage(productData) {
    ...
    products?.forEach(item => {
      ...
      if (categoryIndex === -1) {
        ...
        const modelIndex = data[categoryIndex].models.findIndex(m => m.modelName === model);
        if (modelIndex === -1) {
    ...
    return (
      <div className={`${sidebarOpen || wishListOpen ? " blur-lg" : "block"} bg-white px-4 py-2 pb-8  md:px-6`} >
        <LogoBar />
        <p className="text-[0.70rem] lg:text-lg text-black uppercase tracking-widest mt-2">Home {'>'} Product {'>'} Our Collection</p>
        <div className="">
          <span className="mt-16 flex justify-center text-xl tracking-[0.3rem] md:tracking-[0.5rem] md:mt-32 md:text-2xl lg:text-3xl lg:mt-40">
            OUR COLLECTIONS
          </span>
          <div className="mt-[3rem] md:mt-[5rem] lg:mt-[7rem] flex flex-wrap gap-1 justify-between px-2 lg:flex-nowrap md:justify-center">
            {categories?.map((category, id) => (
              <div key={id} className="w-[21%] md:w-[9%] flex-col">
  
                <Link href={`#${category.category}`} passHref scroll={true}>
              ...
              return (
                <div key={id}>
                  <div className="w-full bg-e3e3e3 p-4 md:p-6 lg:p-8 mt-6 md:mt-9 lg:mt-12 flex flex-col justify-center">
                    <p className="text-center mt-6 md:mt-8 lg:mt-10 flex justify-center text-xl md:text-2xl lg:text-3xl tracking-[0.35rem] md:tracking-[0.5rem] uppercase">
                      {/* {object?.category} */}
                    ...
                    <div className="w-full flex justify-center items-center">
                      <div className="mt-6 md:mt-8 lg:mt-10 flex justify-center w-full lg:w-1/2 pb-8">
                        <p className="text-center text-md md:text-lg tracking-wider">
                          {/* {object?.products[0]?.model?.category.category_text} */}
                          {object?.models[0]?.items[0]?.model.category.category_text}
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex justify-center items-center w-full">
                    <div className={`flex justify-center items-center flex-wrap  gap-6 mt-8 md:mt-12 lg:mt-16 xl:mt-20 `}>
                          ...
                          <div key={i} className="w-[45%] md:w-[30%]">
                            <button onClick={() => handleOpenModal(product)} >
                              <img loading="lazy"
                                onContextMenu={handleContextMenu}
                                onDragStart={handleDragStart}
                                src={product?.items[0].model.model_image_with_background}
                                alt="Image1"
                                className="w-full"
                              />
                            </button>
          ...
          {
            isOpen && 
            <Modal open={isOpen} onClose={() => setIsOpen(false)} data={selectedProduct}  />
          }
