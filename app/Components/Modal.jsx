...
export const Modal = ({ onClose, data }) => {
  ...
  const handleClick = (dim, dimension, side, front, id, amazon, flipkart, obj) => {
  ...
  useEffect(() => {
    // Check if the product ID is already in favorites when component mounts
    ...
    if (favoriteProduct.some((favProd) => favProd.id === currObj.id)) {
      // console.log("Here, included")
      setAddFav(false);
    }
    else {
      // console.log("Here, excluded ")
      setAddFav(true);
    }
  }, [currObj]);
      ...
      <div className="modal">
        ...
        <div className="w-full px-4 flex flex-col items-end md:items-start md:flex-row gap-4">
                ...
                return (
                  ...
                  >
                    <button
                      ...
                      className={`xl:w-12 xl:h-12 lg:w-10 lg:h-10 md:w-8 md:h-8 w-6 h-6 hover:scale-110 overflow-hidden rounded-full ${id === obj.id && "md:border-2 md:p-1"}  `}
                    >
                      <img
          ...
          <div className=" order-first md:-order-first w-full md:w-2/3 flex justify-center items-center flex-col">
            <ModalCarousel data={selectedProduct} showMeasureMent={showMeasureMent} />
            <p className='hidden md:block text-sm sm:text-sm md:text-lg lg:text-xl ml-4 self-start tracking-widest font-extralight text-gray-600 uppercase'>
              {data?.items[0]?.model?.category?.category} {">"} {data?.modelName}
            </p>
          </div>
        </div>
        <div className="w-full flex pb-4 md:mt-4 mt-0 gap-4 justify-center ">
          <div className="w-4/5 md:w-1/3 flex flex-col px-6 gap-1">
            <button className={`  flex justify-between items-center`} onClick={() => setShowMeasureMent(!showMeasureMent)}>
              <h1 className={` ${showMeasureMent ? "text-black" : " text-gray-400 "} tracking-widest text-[0.60rem] sm:text-sm md:text-md xl:text-lg md:font-extralight uppercase`}>
                Show Measurements
              </h1>
          ...
          <div className=" w-[22%] md:w-2/3 gap-2 md:gap-4 flex justify-evenly items-center mr-6">
            ...
            {
              amazon && <div className="xl:w-12 p-1 md:p-2 xl:h-12 lg:w-10 lg:h-10 w-12 h-8 rounded-full overflow-hidden border-2 ">

                <Link href={amazon === null ? "#" : amazon} target="_blank" rel="noopener noreferrer">
                  <Image
                    src={amazonIcon}
                    alt=""
            ...
            {
              flipkart && <div className="xl:w-12 p-1 md:p-2 xl:h-12 lg:w-10 lg:h-10 w-12 h-8 rounded-full overflow-hidden border-2 ">
                <Link href={flipkart === null ? "#" : flipkart} target="_blank" rel="noopener noreferrer">
                  <Image
                    src={flipkartIcon}
                    alt=""
                    className="object-cover w-full h-full"
