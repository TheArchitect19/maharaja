...
export default function GalleryPage(galleryData) {
  ...
  const [subSelectedImage, selectSubSelectedImage] = useState(assets[0].models[0]);
  ...
  return (
    ...
    >
      <LogoBar />
      <p className="text-[0.70rem] lg:text-l text-black uppercase tracking-widest mt-2">Home {'>'} lifestyles </p>
      ...
      <div className="w-full mt-4 md:mt-8 flex justify-start">
        <div className=" w-full md:w-10/12 ml-0 md:md-8 lg:ml-12 flex flex-col ">
          ...
          <div className="my-6 md:my-8 px-6 md:px-10 lg:px-12 text-xl text-gray-100 font-light">
            <p className=" text-justify text-pColor tracking-wider text-sm md:text-md  md:leading-9">
      ...
      <div className="w-full lg:w-[90%] mt-4 md:mt-6 lg:mt-8 flex flex-col md:flex-row justify-center h-auto ">
        <div className="flex flex-col md:hidden h-auto w-full gap-2 p-2">
          <div className="flex gap-6 md:gap-8 lg:gap-10  p-2 md:p-4 lg:p-6">
            <div className=" border-2 border-gray-500 w-20 h-20 md:h-26 md:w-26 lg:w-32 lg:h-32 flex justify-center items-center">
              <div className="bg-e3e3e3 w-16 h-16 md:w-20 md:h-20 lg:w-24 lg:h-24"></div>
            </div>
            <p className="font-bold tracking-wider text-xl md:text-2xl lg:text-3xl xl:text-4xl uppercase pt-4">
              Products
            </p>
          </div>
        ...
        <div className=" w-full  md:w-2/5 md:min-h-[50vh]">
          <div className="hidden md:flex h-auto w-full gap-6 md:gap-8 lg:gap-10  p-2 md:p-4 lg:p-6">
            <div className=" border-2 border-gray-500 w-20 h-20 md:h-26 md:w-26 lg:w-32 lg:h-32 flex justify-center items-center">
            ...
            <div className="flex flex-col w-full items-center">
              {
                <>
                  {/* Spacer */}
                  <div
                    className={`h-[7.2rem] hidden ${dialogueOpen ? "md:hidden" : "md:block"
                      }`}
                  ></div>

                  {/* Dialogue box */}
                  <div
                    className={`hidden ${dialogueOpen ? "md:flex" : "md:hidden"
                  ...
                  >
                    ...
                    <div className={`flex ${selectedImage?.models?.length < 5 && "justify-center"} w-full h-36 overflow-auto`}>
                      { selectedImage?.models?.length !== 0 ? selectedImage?.models?.map((model, id) => (
                        <div key={id} className="min-w-20 flex flex-col items-center justify-center">
                          <img
                          ...
                          />
                          <h3 className="text-[0.65rem] uppercase tracking-wider">{model.model}</h3>
                        </div>
                      )) : 
                        <div className="flex justify-center items-center text-xl font-semibold text-gray-700" style={{padding: "1rem 2rem"}}>
              ...
              <div className="relative grid justify-center items-center grid-cols-4 gap-4 mt-4">
                ...
                <div
                  className={`${dialogueOpen ? "flex" : "hidden"
                    } md:hidden absolute w-full  gap-0 bg-e3e3e3 flex-col py-2 rounded-md h-48 `}
                >
                  <div
                    className=" flex mr-2 justify-end cursor-pointer"
                    onClick={() => setDialogueOpen(false)}
                  ...
                  <div className={`flex ${selectedImage?.models?.length < 3 && "justify-center"} w-full h-40 overflow-auto`}>
                    {selectedImage?.models?.length !== 0 ? selectedImage?.models?.map((model, id) => (
                      <div key={id} className=" min-w-28 flex flex-col items-center justify-center">
                        <img
                          src={model.model_image_no_background}
                          alt="random"
                          onClick={() => selectSubSelectedImage(model)}
                          ...
                          className="h-4/5 cursor-pointer hover:scale-110 transform transition-transform ease-in"
                        />
                        <h3 className="mt-2 text-[0.65rem] text-center uppercase tracking-wider">{model.model}</h3>
                      </div>
                    )) :  
